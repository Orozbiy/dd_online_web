...
class _WelcomeScreenState extends State<WelcomeScreen>
  ...
  Widget _buildButtons(AppLocalizations loc, bool isDark) {
    // Windows'та (desktop) — Сатуучу баскычы гана
    if (!kIsWeb && defaultTargetPlatform == TargetPlatform.windows) {
      return _buildSellerButton(loc);
    }
    ...
    // Веб жана мобилде — Google + Сатуучу
    return Column(
      children: [
        // Google баскычы
        GestureDetector(
        ...
        // Веб болсо Сатуучу баскычын да көрсөт
        if (kIsWeb) ...[
          const SizedBox(height: 12),
          _buildSellerButton(loc),
        ],
      ],
    );
  }

  Widget _buildSellerButton(AppLocalizations loc) {
